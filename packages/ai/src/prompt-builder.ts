...
- Cause of Loss: ${claim.causeOfLoss || 'N/A'}
- Description: ${claim.description}
- Status: ${claim.status}
- Deductible: ${claim.deductible ? `$${claim.deductible}` : 'N/A'}
- Estimated Value: ${claim.estimatedValue ? `$${claim.estimatedValue}` : 'N/A'}
- Approved Value: ${claim.approvedValue ? `$${claim.approvedValue}` : 'N/A'}`;
  }
...
- Address: ${customer.address || 'N/A'}`;
  ...
  private formatInterviewSection(interview: any): string {
    if (!interview.responses) return '## INTERVIEW RESPONSES\nNo interview responses available';
...
- Interview Number: ${interview.interviewNumber || 'N/A'}
- Template: ${interview.templateName || 'N/A'}
- Completed: ${interview.completedAt || 'N/A'}
...
${supplementLines}`;
  ...
  private formatDocumentsSection(documents: any[]): string {
    const docLines = documents.map(doc => {
      return `- ${doc.name} (${doc.type || 'Unknown'}) - Uploaded: ${doc.uploadedAt || 'N/A'}`;
    }).join('\n');
...
${docLines}`;
  ...
  private formatPhotosSection(photos: any[]): string {
    const photoLines = photos.map(photo => {
      return `- ${photo.description || 'Untitled'} - Location: ${photo.location || 'N/A'} - Uploaded: ${photo.uploadedAt || 'N/A'}`;
    }).join('\n');
...
${photoLines}`;
  ...
  private formatActivityTimelineSection(activities: any[]): string {
    const activityLines = activities.slice(-10).map(activity => {
      return `- ${activity.createdAt || 'N/A'}: ${activity.description || 'No description'} by ${activity.userName || 'Unknown'}`;
    }).join('\n');
